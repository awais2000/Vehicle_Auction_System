...
export const contactFrom = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO tbl_contact_us (subject, email, contactNumber, description) VALUES (?, ?, ?, ?)`, 
            [subject, email, contactNumber, description]);


            const id = result.insertId;
        const [contact] = await pool.query(`SELECT * FROM tbl_contact_us WHERE id = ?`, [id]);
...
export const getContactUs = async (req, res) => {
    try {
        const [result] = await pool.query(`SELECT * FROM tbl_contact_us and status = 'Y' `);
...
export const updateContactUs = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`UPDATE tbl_contact_us SET subject = ?, email = ?, contactNumber = ?, description = ?, status = ? WHERE id = ?`, 
